...
class DualSDMCC(nn.Module):
    ...
    def compute_losses(self, x_RNA, x_ADT,x_ATAC, sim_z, dist_z, fused_z,atac, combined_raw ,pro):
        ...
        total_loss = (self.beta * (l_reconstruction +l_reconstruction_sim+l_reconstruction_dist+l_reconstruction_adt+l_reconstruction_atac)  + 20 * l_spatial_reg +reg_loss)
...
spatial_file = "your_path"
spatial_df = pd.read_csv(spatial_file, header=None)
spatial_df.columns = ["barcode", "in_tissue", "array_row", "array_col", "pxl_X", "pxl_Y"]
spatial_df.set_index("barcode", inplace=True)

common_obs = list(set(adata_ADT.obs_names).intersection(
    set(adata_RNA.obs_names)).intersection(
        set(adata_ATAC.obs_names)).intersection(
            set(spatial_df.index)))

adata_ADT = adata_ADT[common_obs].copy()
adata_RNA = adata_RNA[common_obs].copy()
adata_ATAC = adata_ATAC[common_obs].copy()

matched_spatial = spatial_df.loc[adata_ADT.obs_names]
spatial_coords = matched_spatial[["pxl_X", "pxl_Y"]].values.astype(np.float32)
...
sim_edge_weight_vals = similarity_matrix[adjacency_matrix > 0]
# Prevent negative edge weights from causing NaN in GCNConv degree normalization
sim_edge_weight_vals = np.maximum(sim_edge_weight_vals, 0.0)
sim_edge_weight = torch.tensor(sim_edge_weight_vals, dtype=torch.float).to(device)
