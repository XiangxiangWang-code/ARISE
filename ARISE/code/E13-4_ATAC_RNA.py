...
class DualSDMCC(nn.Module):
    ...
    def spatial_regularization_loss(self, emb, dist_edge_index, dist_edge_weight):
        ...
        # Construct adjacency matrix (graph_nei) and negative sample matrix (graph_neg)
...
adata_RNA = sc.read_h5ad(RNA)
adata_ATAC = sc.read_h5ad(ATAC)

print(adata_RNA)
print(adata_ATAC)

sc.pp.filter_genes(adata_RNA, min_cells=100)
print(adata_RNA)
...
ATAC_expression =adata_ATAC[:, adata_ATAC.var['highly_variable']].X
RNA_expression =adata_RNA[:, adata_RNA.var['highly_variable']].X


print(RNA_expression.shape)
print(ATAC_expression.shape)
q = ATAC_expression.shape[1]


x = adata_RNA.obs['x_pxiel'].values
y = adata_RNA.obs['y_pxiel'].values

cell_positions = np.vstack([x, y]).T
...
x_ADT = torch.tensor(ATAC_expression, dtype=torch.float).to(device)
...
def evaluate_dbch_index(embeddings, labels):
    dbch = calinski_harabasz_score(embeddings, labels)
    return dbch
def evaluate_davies_bouldin_index(embeddings, labels):
...
for epoch in range(num_epochs):
    ...
    silhouette_avg = evaluate_silhouette_score(embeddings, predicted_labels)
    dbi = evaluate_davies_bouldin_index(embeddings, predicted_labels)
